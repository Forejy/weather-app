async function appendFirstPart() {
  ...
  const celestialContainer = document.getElementById("js-celestial-container");
  const cloudsContainer = document.getElementById("js-cloud-container");
  const cloudsFilterWhite = document.getElementById("filter-white__turbulence");

  if (weather === 800 || weather === 801) {
    if (dayOrNight === 'd') {
      celestialContainer.classList.add("sun-container");
      celestialContainer.firstChild.classList.add("sun");
    }
    else {
      celestialContainer.classList.add("moon-container");
      celestialContainer.firstChild.classList.add("moon");
    }
    celestialContainer.classList.remove("d-none");
  }

  if (weather === 801) {
    cloudsContainer.classList.add("few-clouds");
    celestialContainer.classList.add("celestial-with-few-clouds");
    cloudsFilterWhite.setAttribute("seed", 3);
    cloudsContainer.classList.remove("d-none");
  }
  else if (weather === 802) {
    cloudsContainer.classList.add("cloud-container-one-cloud");
    cloudsContainer.classList.remove("d-none");
  }
  else if (weather === 803) {
    cloudsFilterWhite.setAttribute("baseFrequency", 0.007);
    cloudsContainer.classList.add("cloud-container-broken-clouds");
    cloudsContainer.firstChild.classList.add("broken-clouds");
    cloudsContainer.classList.remove("d-none");
  }
}
...
async function appendWeathersToWeek() {
  ...
  for(let i = 0; i < daysInWeek; i++) {
    tempCont.firstChild.innerText = days[i];
...
function transformDayOrWeek() {
  ...
  function changeSwitchButton() {
    ...
    ;
    let words = document.getElementsByClassName("js-day-week__text__words");
